...
def randlogin():
    import urllib
    import urllib2
    data = urllib.urlopen('http://ftp.vistatv.online/codes.php?server=9&ip=aGFoYWZ1Y2t1aXZ1ZWZvcmF0dGFja2luZ21lZm9ybm9yZWFzb24=&access=bG1hb3dqeWlza29kaWNvbW1uaXR5IGlrZXRoaXM=').read()
    data = data.split(",")
    #xbmc.log(str(data),2)
    import random
    servers = data 
...
kasutajanimi=userdata[0]
salasona=userdata[1]
#salasona=kasutajanimi=plugintools.get_setting("Username")
...
def peamenyy(params):
    ...
    if channels == 1 and GoDev.mode != 5 and GoDev.mode != 1:
        ...
        plugintools.add_item( action=vod_channels("c2VjdXJpdHlfY2hlY2s="),  title="[COLOR gold][B][I]EPTV Preview 2[/I][/B][/COLOR]", thumbnail=os.path.join(LOAD_LIVE,vod_channels("bGl2ZXR2LnBuZw==")) , fanart=os.path.join(LOAD_LIVE,vod_channels("YmFja2dyb3VuZC5wbmc=")) ,  folder=True )
