    ...
    def get_dvr_link(self, get_scheduleid, quality_type, recordings_quality):
	    ...
	    for i in results:
	        try:
		        ...
		        json = self._get_json('stream/1/dvr/play', {'token': self.token, 'key': self.passkey, 'scheduleid': i['scheduleid']})
                        stream = json['stream']
	                if recordings_quality == '950':
		            url = stream
                        else:
		            url = stream.replace('smil:', 'mp4:').replace('.smil', '_' + str(recordings_quality) + '.mp4')
		        if Addon.get_setting('free_package') == 'true':
		            if name in ['CW','ABC','FOX','PBS','CBS','NBC','MY9']:
		                channels.append({
