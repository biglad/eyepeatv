...
class sources:
    ...
    def sourcesFilter(self):
        ...
        for i in range(len(self.sources)):
            ...
            if s.lower() == 'cached' or s.lower() == 'torrent':
                s = '[bolor=orange]Torrent[/bolor]'
                #d = 'TOR'
            if s.lower() == 'deb-cached': 
                s = '[bolor=orange]Torrent[/bolor]'
                #d = 'TOR'
				
            if q.lower() == "4k":
               q = "[bolor=orange]"+q+"[/bolor]"
               #d = "prem"
			
            if not d.lower():
                if "olpair" in s.lower():
                    s = s+" - [bolor=red]([I]Pair Host[/I] )[/bolor]"
                if "openl" in s.lower():
                    s = s+" - [bolor=red]([I]Pair Host[/I] )[/bolor]"
                if "openload.co " in s.lower():
                    s = s+" - [bolor=red]([I]Pair Host[/I] )[/bolor]"
                if "openload.io " in s.lower():
                    s = s+" - [bolor=red]([I]Pair Host[/I] )[/bolor]"
                if "thevideo" in s.lower():
                    s = s+" - [bolor=red]([I]Pair Host[/I] )[/bolor]"
                if "thevideo.me" in s.lower():
                    s = s+" - [bolor=red]([I]Pair Host[/I] )[/bolor]"
                if "flashx" in s.lower():
                    s = s+" - [bolor=red]([I]Pair Host[/I] )[/bolor]"
                if "vshare" in s.lower():
                    s = s+" - [bolor=red]([I]Pair Host[/I] )[/bolor]"
                if "vidup" in s.lower():
                    s = s+" - [bolor=red]([I]Pair Host[/I] )[/bolor]"
            ...
            if "vidzi" in s.lower():
                s = s+" [bolor=green]** GOOD HOST **[/bolor]"
            if "vidzi.to" in s.lower():
                s = s+" [bolor=green]** GOOD HOST **[/bolor]"
            if "streamago" in s.lower():
                s = s+" [bolor=green]** GOOD HOST **[/bolor]"
            if "treamango" in s.lower():
                s = s+" [bolor=green]** GOOD HOST **[/bolor]"
            if "directlink" in s.lower():
                s = s+" [bolor=green]** GOOD HOST **[/bolor]"
            if "dl" in s:
                s = s+" [bolor=green]** GOOD HOST **[/bolor]"
            if "cdn" in s:
            ...
            self.sources[i]['label'] = self.sources[i]['label'].replace('[BOLOR=GREEN]', '[COLOR=green]').replace('[/BOLOR]', '[/COLOR]')  
            self.sources[i]['label'] = self.sources[i]['label'].replace('[BOLOR=RED]', '[COLOR=red]').replace('[/BOLOR]', '[/COLOR]')   
            self.sources[i]['label'] = self.sources[i]['label'].replace('[BOLOR=ORANGE]', '[COLOR=orange]').replace('[/BOLOR]', '[/COLOR]') 
            self.sources[i]['label'] = self.sources[i]['label'].replace('[BOLOR=WHITE]', '[COLOR=white][I]').replace('[/BOLOR]', '[/COLOR][/I]') 
    ...
    def getConstants(self):
        self.itemProperty = 'plugin.video.vistatvshowbox.container.items'

        self.metaProperty = 'plugin.video.vistatvshowbox.container.meta'
        ...
        sourceDir2 = sources()
        from vistascrapers import sources
        sourceDir3 = sources()

        try:
            if scraperSetting == 'Vista Scrapers':
                self.sourceDict = sourceDir3
                self.module_name = control.addon('script.module.vistatvshowbox').getSetting('module.provider')
            if scraperSetting == 'OPEN-Scrapers Scrapers':
            ...
            if scraperSetting == 'Vista + Built-in':
                self.sourceDict = sourceDir2 + sourceDir3
                self.module_name = 'Int & OS - '+control.addon('script.module.vistatvshowbox').getSetting('module.provider')
            if scraperSetting == 'OPEN-Scrapers + Built-in':
                self.sourceDict = sourceDir1 + sourceDir2
                self.module_name = 'Int & LS - '+control.addon('script.module.openscrapers').getSetting('module.provider')
        except: return
