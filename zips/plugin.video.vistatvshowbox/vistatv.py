...
def syncTraktLibrary():
    control.execute(
        'RunPlugin(plugin://%s)' % 'plugin.video.vistatvshowbox/?action=tvshowsToLibrarySilent&url=traktcollection')
    control.execute(
        'RunPlugin(plugin://%s)' % 'plugin.video.vistatvshowbox/?action=moviesToLibrarySilent&url=traktcollection')
...
if action == None:
    ...
    from resources.lib.modules import control
    control.openSettings('0.0', 'script.module.vistatvshowbox')
    ...
    try:
        ...
        if rtype == "movie":
            ...
            try: control.infoDialog(urllib.unquote(rlist[rand]['tvshowtitle']).decode('utf8')+" - Season "+rlist[rand]['season']+" - "+rlist[rand]['title'], control.lang(32536).encode('utf-8'), time=30000)
            except: pass
