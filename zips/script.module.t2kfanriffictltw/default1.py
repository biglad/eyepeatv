...
def list_update():
	...
	try:
		if window.getFocus() == buildlist:
			...
			buildtextbox.setLabel('[COLOR %s]Theme Server Selected: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR, name))
			...
			desctextbox.setText('[COLOR %s]Description: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,description))
...
def BuildList():
	...
	if not BUILDFILE == 'http://' and not BUILDFILE == '':
		...
		PreviewButton = pyxbmct.Button('[COLOR %s][B]Hello[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		window.placeControl(PreviewButton,20 , 20, 8, 8)
		window.connect(PreviewButton,lambda: buildVideo(Bname))
		
		InstallButton = pyxbmct.Button('[COLOR %s][B]Please Click[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		window.placeControl(InstallButton,28 , 20, 8, 8)
		window.connect(InstallButton, lambda: buildWizard(Bname,'normal'))
		
		FreshStartButton = pyxbmct.Button('[COLOR %s][B]Below[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		...
		buildlist.controlRight(ThemeButton)
		...
		ThemeButton.controlUp(buildlist)
