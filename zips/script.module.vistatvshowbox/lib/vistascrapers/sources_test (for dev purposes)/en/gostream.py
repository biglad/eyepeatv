...
class source:
    ...
    def resolve(self, url):
        try:
            ...
            p1 = self.scraper.post('https://gostream.sc/ip.file/swf/plugins/ipplugins.php', data=post).content
            p1 = json.loads(p1)
            p2 = self.scraper.get(
                'https://gostream.sc/ip.file/swf/ipplayer/ipplayer.php?u=%s&s=%s&n=0' %
                (p1['s'],
                 urldata['data-server'])).content
            p2 = json.loads(p2)
            p3 = self.scraper.get('https://gostream.sc/ip.file/swf/ipplayer/api.php?hash=%s' % (p2['hash'])).content
            p3 = json.loads(p3)
            n = p3['status']
            if n is False:
                p2 = self.scraper.get(
                    'https://gostream.sc/ip.file/swf/ipplayer/ipplayer.php?u=%s&s=%s&n=1' %
                    (p1['s'],
                     urldata['data-server'])).content
                p2 = json.loads(p2)
