...
def setSurfaceCodec():
    if surfaceCodec == False:
        ...
        utils.dialog.ok('VistaTV', "Media Codec (Surface) Enabled", "For Best Playback (needed for x265)")
    else:
        xbmc.executeJSONRPC(jsonSetSurfaceCodec  % "false")
        utils.dialog.ok('VistaTV', "Media Codec (Surface) Disabled", "Please restart ivue for changes to take affect")

def setCodec():
    if codec == False:
        xbmc.executeJSONRPC(jsonSetCodec % "true")
        utils.dialog.ok('VistaTV', "Media Codec Enabled", "Needed for live streaming")
    else:
        xbmc.executeJSONRPC(jsonSetCodec  % "false")
        utils.dialog.ok('VistaTV', "Media Codec Disabled", "Please restart ivue for changes to take affect")
